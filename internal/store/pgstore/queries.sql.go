...
const getRoomMessage = `-- name: GetRoomMessage :many
SELECT 
    "id", "room_id", "message", "reaction_count","answered"
FROM messages
WHERE 
...
func (q *Queries) GetRoomMessage(ctx context.Context, roomID uuid.UUID) ([]GetRoomMessageRow, error) {
	rows, err := q.db.Query(ctx, getRoomMessage, roomID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetRoomMessageRow
	for rows.Next() {
		var i GetRoomMessageRow
		if err := rows.Scan(
			&i.ID,
			&i.RoomID,
			&i.Message,
...
const removeReactToMessage = `-- name: RemoveReactToMessage :one
UPDATE messages
SET
    reaction_count = reaction_count - 1
WHERE
...
func (q *Queries) RemoveReactToMessage(ctx context.Context, id uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, removeReactToMessage, id)
	var reaction_count int64
	err := row.Scan(&reaction_count)
	return reaction_count, err
}
